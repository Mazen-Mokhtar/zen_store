...
import { ConfirmDTO, confrimForgetPasswordDTO, ForgetPasswordDTO, loginDTO, ReSendCodeDTO, ResetPasswordDTO, SignupDTO } from './dto';
...
export class AuthService {
  ...
  async reSendCode(body: ReSendCodeDTO) {
    ...
    if (!user)
      throw new ConflictException(messageSystem.user.invalid)
    const generateOTP = Randomstring.generate({ length: 6, charset: '1234567890' });
  ...
  async forgetPassword(body: ForgetPasswordDTO) {
    ...
    if (!user)
      throw new BadRequestException(messageSystem.user.invalid)
    const generateOTP = Randomstring.generate({ length: 6, charset: '1234567890' });
    this.emailEvent.sendEmail(body.email, generateOTP, user.userName);
    user.code = generateHash(generateOTP)
    user.codeExpiration = new Date(Date.now() + 3 * 60 * 1000)
  ...
  async confirmForgetPassword(body: confrimForgetPasswordDTO) {
    const user = await this.userRepository.findByEmail({ email: body.email, isConfirm: true })
    if (!user)
  ...
  async resetPassword(body: ResetPasswordDTO) {
    const payload = this.tokenService.verify(body.resetToken, "", ITokenTypes.reset);
